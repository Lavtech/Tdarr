...
search_description.grid(row=6, column=0, columnspan=2, padx=20, pady=10, sticky="w")
...
def search_directory(directory=None):
    ...
    if file_count > 0:  # Show labels and remove button only if files are found
        remove_button.grid()
        total_size_label.grid()
        file_count_label.grid()
    total_size_label.configure(text=f"Total size of found files: {format_size(total_file_size)}")
    file_count_label.configure(text=f"Total number of files found: {file_count}")
...
def remove_files():
    if messagebox.askyesno("Confirm Delete", "Are you sure you want to delete these files?"):
        ...
        remove_button.grid_remove()  # Hide the button again
        total_size_label.configure(text="Total size of found files: 0 bytes")
        file_count_label.configure(text="Total number of files found: 0")



# Label for Ignore these subfolders
ignore_label = customtkinter.CTkLabel(app, text="Ignore these subfolders:")
ignore_label.grid(row=4, column=0, columnspan=1, padx=20, pady=10, sticky="w")
...
featurettes_checkbox.grid(row=5, column=0, columnspan=1, padx=20, pady=10, sticky="w")
...
fanart_checkbox.grid(row=5, column=1, columnspan=1, padx=0, pady=0, sticky="w")


# Search button
search_button = customtkinter.CTkButton(app, text="Search", command=search_directory)
search_button.grid(row=7, column=0, columnspan=1, padx=20, pady=10, sticky="ew")


# Button to remove found files
remove_button = customtkinter.CTkButton(app, text="Remove These Files?", command=remove_files)
remove_button.grid(row=8, column=0, columnspan=1, padx=20, pady=10, sticky="ew")
remove_button.grid_remove()  # Initially hide the button
...
total_size_label.grid(row=9, column=0, columnspan=1, padx=10, pady=10, sticky="w")
total_size_label.grid_remove()  # Initially hide the label

# Label to display the total number of found files
file_count_label = customtkinter.CTkLabel(app, text="Total number of files found: 0")
file_count_label.grid(row=9, column=1, columnspan=1, padx=10, pady=10, sticky="w")
file_count_label.grid_remove()  # Initially hide the label

# Textbox to display found files
files_found = customtkinter.CTkTextbox(app, height=10)
files_found.grid(row=0, column=2, rowspan=11, columnspan=2, padx=20, pady=10, sticky="nsew")
...
app.grid_rowconfigure(10, weight=1)  # Ensure the files display fills from top to bottom
